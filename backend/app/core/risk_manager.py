...
class RiskManager:
    def __init__(self, account_equity):
        ...
        logging.info(f"RiskManager initialized with account equity of: ${self.account_equity:,.2f}")
    ...
    def calculate_position_size(self, entry_price, stop_loss_price, risk_percentage=0.01):
        ...
        if risk_per_share <= 0:
            logging.warning("Stop loss price must be less than the entry price for a long position.")
            return 0

        capital_to_risk = self.account_equity * risk_percentage
        position_size = capital_to_risk / risk_per_share
        
        logging.info(f"Position size calculated: {position_size:.4f} shares. (Risking ${capital_to_risk:,.2f})")
    ...
    def calculate_stop_loss(self, entry_price, atr_value, atr_multiplier=2.0):
        ...
        logging.info(f"Calculated dynamic stop loss at: ${stop_loss_price:,.2f} (Entry: ${entry_price:,.2f}, ATR: {atr_value:,.2f}) ")
