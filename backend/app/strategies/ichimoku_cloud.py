...
class IchimokuCloudStrategy(BaseStrategy):
    ...
    async def run_on_trade(self, trade):
        ...
        bars = self.alpaca_service.get_bars(symbol, timeframe, limit=self.senkou + 5).df
        logging.info(
        ...
        logging.info(
            f"Ichimoku Cloud data for {symbol}: Close={latest_close:.2f}, Tenkan={latest_tenkan:.2f}, Kijun={latest_kijun:.2f}, Senkou A={latest_senkou_a:.2f}, Senkou B={latest_senkou_b:.2f}"
        )

        current_position = await self.get_position(symbol)

        if (
            latest_tenkan > latest_kijun
            and latest_close > latest_senkou_a
            and latest_close > latest_senkou_b
