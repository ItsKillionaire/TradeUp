...
class MomentumStrategy(BaseStrategy):
    def __init__(self, alpaca_service, telegram_service, google_sheets_service, length=14, trade_percentage=0.05, take_profit_pct=0.05, stop_loss_pct=0.02):
    ...
    async def run(self, symbol: str, timeframe: str, db: Session):
        ...
        bars.ta.roc(length=self.length, append=True)

        latest_roc = bars[f'ROC_{self.length}'].iloc[-1]
        logging.info(f"Latest ROC for {symbol}: {latest_roc:.2f}")

        current_position = await self.get_position(symbol)

        if latest_roc > 0 and current_position == 0:
            message = f"Buy signal for {symbol} (Momentum)"
        ...
        elif latest_roc < 0 and current_position > 0:
            message = f"Sell signal for {symbol} (Momentum)"
            logging.info(message)
            await self.telegram_service.send_message(message)
            await manager.broadcast_json({"type": "log", "message": message})
            ...
            message = f"No signal for {symbol} (Momentum) or already in position"
            logging.info(message)
            await manager.broadcast_json({"type": "log", "message": message})
