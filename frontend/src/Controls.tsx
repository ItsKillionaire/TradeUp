...
const Controls: React.FC = () => {
    ...
    const handleStop = () => {
        axios.post<StartStopResponse>(`/api/strategy/stop/${strategy}/${symbol}`)
            .then((response) => {
                console.log(response.data.message);
        ...
        <Box sx={{ flexGrow: 1 }}>
            ...
            <Grid container spacing={2} alignItems="center">
                        ...
                        <FormControl variant="outlined" fullWidth>
                            ...
                            >
                                {availableStrategies.map((strat) => (
                                    <MenuItem key={strat.name} value={strat.name}>{strat.display_name}</MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                        {selectedStrategy && (
                ...
                <Grid item xs={12} sm={6} md={2}>
                    <Button variant="contained" color="primary" onClick={handleStart} disabled={botStatus === 'online' || !strategy} fullWidth>
                        Start Bot
                    </Button>
                </Grid>
                <Grid item xs={12} sm={6} md={2}>
                    <Button variant="contained" color="secondary" onClick={handleStop} disabled={botStatus !== 'online' || !strategy} fullWidth>
                        Stop Bot
                    </Button>
                </Grid>
