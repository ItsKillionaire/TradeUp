...
const AccountInfo: React.FC = () => {
    ...
    const dailyChange = parseFloat(account.equity) - parseFloat(account.last_equity);
        ...
        <Box sx={{ flexGrow: 1 }}>
            ...
            <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <Paper elevation={3} sx={{ p: 2, height: '100%' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                            {isProfit ? <TrendingUp sx={{ mr: 1, color: 'success.main' }} /> : <TrendingDown sx={{ mr: 1, color: 'error.main' }} />}
                            <Typography variant="subtitle1">Today's P/L</Typography>
                        </Box>
                        <Typography variant="h5" color={isProfit ? 'success.main' : 'error.main'}>${dailyChange.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</Typography>
                    </Paper>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Paper elevation={3} sx={{ p: 2, height: '100%' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                            <Power sx={{ mr: 1, color: account.status === 'ACTIVE' ? 'success.main' : 'error.main' }} />
