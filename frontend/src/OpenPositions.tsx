                ...
                <Table stickyHeader sx={{ minWidth: 650 }} aria-label="simple table">
                    ...
                    <TableBody>
                        {positions.map((position: any) => {
                            ...
                            const takeProfitPrice = position.take_profit_price ? parseFloat(position.take_profit_price) : null;
                            const stopLossPrice = position.stop_loss_price ? parseFloat(position.stop_loss_price) : null;
                            const qty = parseFloat(position.qty);

                            const potentialGain = takeProfitPrice ? (takeProfitPrice - entryPrice) * qty : null;
                            const potentialLoss = stopLossPrice ? (stopLossPrice - entryPrice) * qty : null;
                            ...
                            return (
                                <TableRow key={position.asset_id}>
                                    ...
                                    <TableCell align="right">
                                        {stopLossPrice ? `${stopLossPrice.toFixed(2)} (${potentialLoss?.toFixed(2)})` : 'N/A'}
                                    </TableCell>
                                </TableRow>
                            );
                        })}
